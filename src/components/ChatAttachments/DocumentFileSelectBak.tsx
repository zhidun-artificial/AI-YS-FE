...
import { LibraryItem } from '@/services/libraries/createLibrary';
...
interface DocumentDirNodeData extends LibraryItem, Omit<TreeDataNode, 'title'> {
  docNodeType: 'dir';
  children?: DocumentTreeNodeData[];
}

interface DocumentFileNodeData extends DocItem, Omit<TreeDataNode, 'title'> {
  docNodeType: 'file';
}

type DocumentTreeNodeData = DocumentDirNodeData | DocumentFileNodeData;

const updateTreeData = (
  list: DocumentTreeNodeData[],
  key: React.Key,
  children: DocumentTreeNodeData[],
): DocumentTreeNodeData[] =>
  list.map((node) => {
    if (node.key === key) {
      return {
        ...node,
    ...
    if (node.docNodeType === 'dir' && node.children) {
      return {
        ...node,
...
const DocumentFileSelect: React.FC<DocumentFileSelectProps> = ({
  ...
  //   const checkedKeys = treeRef.current?.state?.checkedKeys as React.Key[];
  //   if (checkedKeys.includes(node.id)) {
  //     setCheckedKeys(() => checkedKeys.filter((key) => `${node.id}` !== key));
  //     setDocFiles(() => docFiles.filter((item) => `${item.id}` !== `${node.id}`));
  //   } else {
  //     setCheckedKeys(() => [...checkedKeys, node.id]);
  ...
  //       const newDocFiles = newCheckedKeys.includes(node.id)
  //         ? [...prevDocFiles, { id: `${node.id}`, fileName: node.fileName, url: node.url }]
  //         : prevDocFiles.filter((file) => `${file.id}` !== `${node.id}`);

  //       if (onDocSelectChange) onDocSelectChange(newDocFiles);
  //       return newDocFiles;
  //     });
  ...
  //   <div className="flex items-center" onClick={disabled ? undefined : () => onTitleClick(item)}>
  ...
  const onNodeClick = (node: DocumentTreeNodeData) => {
    if (disabled) return;
  ...
  const renderTreeNodes = (data: DocumentTreeNodeData[]) =>
    data.map((item) => {
      if (item.docNodeType === 'dir') {
        return (
          <Tree.TreeNode
        ...
        <Tree.TreeNode
          title={
            <div
              ...
              onClick={disabled ? undefined : () => onNodeClick(item)}
            >
              {getFileIcon(item.fileName)}
              <span
                className="ml-2 max-w-[300px] line-clamp-1 whitespace-pre"
